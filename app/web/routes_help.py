...
GUIDES: tuple[dict, ...] = (
    ...
    },
    {
        ...
        "summary": "Flusso base per selezionare documenti aperti, usare eventuali note di credito e registrare il netto realmente versato.",
        ...
        "steps": [
            ...
            "Se devi compensare del credito del fornitore, scegli una o piu note di credito aperte nel box dedicato: il gestionale le usera` automaticamente fino a coprire il residuo selezionato.",
            "Compila data, metodo di pagamento, IBAN e note; allega il PDF quando disponibile o richiesto.",
            "Controlla il riepilogo lordo, compensazione e netto da versare, poi conferma la registrazione.",
        ],
        "checks": [
            "Il movimento compare nella cronologia pagamenti solo per il netto realmente versato.",
            "La fattura risulta aggiornata con importo residuo o stato pagato corretto.",
            "La nota di credito usata riduce il proprio residuo disponibile nello scadenziario.",
        ],
        "actions": [
            {"label": "Apri Pagamenti", "endpoint": "payments.payment_index"},
            {"label": "Guida note credito", "endpoint": "help.guide_view", "params": {"slug": "gestire-note-di-credito-nei-pagamenti"}},
            {"label": "Apri Scadenziario", "endpoint": "payments.schedule_view"},
        ],
    },
    {
        "slug": "gestire-note-di-credito-nei-pagamenti",
        "icon": "journal-text",
        "title": "Gestire note di credito nei pagamenti",
        "summary": "Come usare una nota di credito per ridurre il netto da pagare senza registrarla come incasso o bonifico negativo.",
        "audience": "Chi compensa note di credito durante la registrazione dei pagamenti.",
        "estimated_time": "3-5 minuti",
        "prerequisites": [
            "Avere almeno una nota di credito aperta nel gestionale.",
            "Selezionare fatture dello stesso fornitore e della stessa intestazione della nota di credito.",
        ],
        "steps": [
            "Apri `Pagamenti` e vai in `Nuovo Pagamento`.",
            "Seleziona le fatture da saldare come fai normalmente.",
            "Nel campo `Note di credito da compensare`, scegli una o piu note aperte dello stesso fornitore.",
            "Controlla il riepilogo: vedrai totale fatture, quota compensata e netto finale da versare.",
            "Conferma il pagamento: il gestionale registra il bonifico solo per il netto e salva l'utilizzo della nota di credito separatamente.",
        ],
        "checks": [
            "La cronologia pagamenti mostra solo il netto bancario.",
            "Il residuo della nota di credito diminuisce o si azzera se usata tutta.",
            "Le fatture compensate interamente non restano tra i documenti aperti.",
        ],
        "actions": [
            {"label": "Apri Pagamenti", "endpoint": "payments.payment_index"},
            {"label": "Apri Scadenziario", "endpoint": "payments.schedule_view"},
